...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            JSONArray featuresArray = reader.optJSONArray("features");

            for(int i = 0; i < featuresArray.length(); i++) {
                JSONObject object = featuresArray.getJSONObject(i);

                JSONObject object1 = object.optJSONObject("properties");
                ...
                for(int a = 0; a < object1.length(); a++) {
                    mag = object1.optString("mag");
                    loc = object1.optString("place");
                    time = object1.optString("time");
                }
